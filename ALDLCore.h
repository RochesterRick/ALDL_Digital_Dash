...
public:
  ...
  void resetToDefaults() {
    ...
    strncpy(settings.ids[1], "CMDF4_COOLANT_TEMP_F", MAX_ID_LENGTH);
    strncpy(settings.ids[2], "CMDF4_BATTERY_VOLT",   MAX_ID_LENGTH);
    strncpy(settings.ids[3], "CMDF4_O2_SENSOR",      MAX_ID_LENGTH);
    strncpy(settings.ids[4], "CMDF4_MAP_KPA",        MAX_ID_LENGTH);
    strncpy(settings.ids[5], "CMDF4_IAC_POSITION",   MAX_ID_LENGTH);

    for (int i = 0; i < NUM_IDS; i++) {
      settings.ids[i][MAX_ID_LENGTH] = '\0';
    }
  }
